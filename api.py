...
def programme():
    ...
    try:
        ...
        for reunion in data.get('programme', {}).get('reunions', []):
            ...
            lieu  = hippo.get('libelleCourt', '') or hippo.get('libelleLong', '')
            # Extraire pays depuis toutes les sources possibles
            pays_hippo = hippo.get('pays') or {}
            if isinstance(pays_hippo, dict):
                pays = pays_hippo.get('code','') or pays_hippo.get('libelle','')
            else:
                pays = str(pays_hippo) if pays_hippo else ''
            if not pays:
                pays = reunion.get('pays','') or reunion.get('codePays','') or                        reunion.get('paysCode','') or reunion.get('paysLibelle','') or                        hippo.get('codePays','') or hippo.get('paysCode','') or ''
            if isinstance(pays, dict):
                pays = pays.get('code','') or pays.get('libelle','')
            pays = str(pays).strip().upper() if pays else ''
            # Déterminer si réunion européenne
            PAYS_EU = {'FR','FRA','BE','BEL','DE','DEU','GB','GBR','IE','IRL',
                       'IT','ITA','ES','ESP','SE','SWE','NO','NOR','DK','DNK',
                       'FI','FIN','NL','NLD','AT','AUT','CH','CHE','CZ','CZE',
                       'HU','HUN','PL','POL','PT','PRT','SK','SVK','SI','SVN',
                       'HR','HRV','LU','LUX','GR','GRC','RO','ROU','BG','BGR',
                       'RS','SRB','MT','MLT','CY','CYP','EE','EST','LV','LVA','LT','LTU',
                       'FRANCE','BELGIQUE','ALLEMAGNE','IRLANDE','ITALIE',
                       'ESPAGNE','SUEDE','NORVEGE','DANEMARK','FINLANDE',
                       'PAYS-BAS','AUTRICHE','SUISSE','LUXEMBOURG',
                       'GRANDE-BRETAGNE','ANGLETERRE','ROYAUME-UNI'}
            # Lieux hors-Europe connus (fallback si pas de code pays)
            LIEUX_NON_EU = {
                'SAN ISIDRO','BUENOS AIRES','PALERMO','LA PLATA',  # Argentine
                'MARONAS',                                           # Uruguay
                'VALPARAISO','HIPODROMO CHILE',                     # Chili
                'HONG KONG','SHA TIN','HAPPY VALLEY',               # Hong Kong
                'SINGAPOUR',                                         # Singapour
                'DUBAI','MEYDAN','ABU DHABI',                       # Emirats
                'TOKYO','NAKAYAMA','KYOTO','HANSHIN','SAPPORO',     # Japon
                'MELBOURNE','SYDNEY','FLEMINGTON','RANDWICK',        # Australie
                'JOHANNESBURG','CAPE TOWN','TURFFONTEIN',           # Afrique du Sud
                'DOHA',                                              # Qatar
                'NEW YORK','SANTA ANITA','BELMONT','CHURCHILL',     # USA
            }
            lieu_up = lieu.upper().strip()
            lieu_non_eu = any(l in lieu_up for l in LIEUX_NON_EU)
            if pays:
                # Code pays connu → vérifier liste blanche EU
                is_european = pays in PAYS_EU
            elif lieu_non_eu:
                # Lieu clairement hors EU identifié
                is_european = False
            else:
                # Pas de code pays ni lieu connu → européen par défaut
                is_european = True
        ...
        if reunion.get('numOfficiel') == r_num or reunion.get('numReunion') == r_num:
            for course in reunion.get('courses', []):
                if course.get('numOrdre') == c_num or course.get('numExterne') == c_num:
                    ...
                    hippo = reunion.get('hippodrome', {}) or {}
                    # Extraire le plafond de gains depuis le texte conditions
                    # Ex: "n'ayant pas gagné 580.000" → 580000
                    # Ex: "reçu une allocation de 13.000" → 13000
                    import re as _re
                    conditions_txt = course.get('conditions', '') or ''
                    plafond_gains = 0.0
                    m = _re.search(
                        r"(?:gagn[eé]|re[cç]u(?:\s+une\s+allocation\s+de)?)\s+([\d]+\.[\d]+)",
                        conditions_txt
                    )
                    if m:
                        try:
                            plafond_gains = float(m.group(1).replace('.',''))
                        except:
                            plafond_gains = 0.0
                    return {
                        ...
                        'hippodrome_code': hippo.get('code', '') or hippo.get('codeHippodrome', '') or '',
                        'hippodrome_nom':  hippo.get('libelleCourt', '') or '',
                        'plafond_gains':   plafond_gains,
                    }
    return _cond_vides()
...
def _calculer_proba_win_ev(df_nc, score_brut):
    ...
    - T=2.0 : discrimination modérée, plus proche des taux réels observés
    """
...
def _get_cat_ferrure_generique(f):
    """
    Classe une valeur de ferrure (brute PMU ou entier ferrage_map) en
    FERRE / DEFERRE_PARTIEL / DEFERRE_TOTAL. Mapping validé sur les 10
    valeurs réellement observées dans historique_enrichi.csv :
      FERRE, REFERRE_ANTERIEURS_POSTERIEURS, '0'              -> FERRE
      PROTEGE_ANTERIEURS, PROTEGE_POSTERIEURS,
      PROTEGE_ANTERIEURS_POSTERIEURS, DEFERRE_ANTERIEURS,
      DEFERRE_POSTERIEURS, DEFERRE_ANTERIEURS_PROTEGE_POSTERIEURS,
      PROTEGE_ANTERIEURS_DEFERRRE_POSTERIEURS                  -> DEFERRE_PARTIEL
      DEFERRE_ANTERIEURS_POSTERIEURS                           -> DEFERRE_TOTAL
    Préférer la colonne deferre_brut (string PMU complète) à deferre
    (entier issu de ferrage_map, incomplet — ne connaît pas les PROTEGE_*).
    """
    f = str(f).strip().upper()
    if f == 'DEFERRE_ANTERIEURS_POSTERIEURS':
        return 'DEFERRE_TOTAL'
    if f in ('FERRE', 'REFERRE_ANTERIEURS_POSTERIEURS', '0', '0.0', 'NAN', 'NONE', ''):
        return 'FERRE'
    if 'PROTEGE' in f or 'DEFERRE' in f:
        return 'DEFERRE_PARTIEL'
    try:
        fi = int(float(f))
...
def _appliquer_cat_ferrure(df_nc):
    """Ajoute df_nc['cat_ferrure'] en préférant deferre_brut à deferre."""
    if 'deferre_brut' in df_nc.columns:
        df_nc['cat_ferrure'] = df_nc['deferre_brut'].apply(_get_cat_ferrure_generique)
    elif 'deferre' in df_nc.columns:
        df_nc['cat_ferrure'] = df_nc['deferre'].apply(_get_cat_ferrure_generique)
    else:
...
def _notes_pmu_plat_v1(df_nc, date_str, r_num, c_num):
    ...
    # ── date_derniere_course — depuis date_derniere_course_plat ────────
    def _get_date_derniere_course_plat(nom):
        return _date_derniere_course_plat.get(str(nom).upper().strip())
    df_nc['date_derniere_course'] = df_nc['nom'].apply(_get_date_derniere_course_plat)
    ...
    for _, row in df_nc.sort_values('note_pmu', ascending=False).iterrows():
        result.append({
            ...
            "gains_carriere": float(row.get('gains_carriere') or 0.0),
            "plafond_gains": float(row.get('plafond_gains') or 0.0),
            ...
            "rk_moyenne_3courses": float(row['rk_moyenne_3courses']) if pd.notna(row.get('rk_moyenne_3courses')) and row.get('rk_moyenne_3courses') else None,
            "rk_meilleur_an": float(row['rk_meilleur_an']) if pd.notna(row.get('rk_meilleur_an')) and row.get('rk_meilleur_an') else None,
            "rk_meilleur_an_date": str(row.get('rk_meilleur_an_date')) if row.get('rk_meilleur_an_date') else None,
            "date_derniere_course": str(row.get('date_derniere_course')) if row.get('date_derniere_course') else None,
            "place_corde": int(row['place_corde']) if pd.notna(row.get('place_corde')) else None,
            "apt_terrain_actuel": round(float(row['apt_terrain_actuel']) * 100, 1) if pd.notna(row.get('apt_terrain_actuel')) else None,
            "apt_dist_recente": round(float(row['apt_dist_recente']) * 100, 1) if pd.notna(row.get('apt_dist_recente')) else None,
            "aptitude_hippodrome": round(float(row['aptitude_hippodrome']) * 100, 1) if pd.notna(row.get('aptitude_hippodrome')) else None,
            "jockey_win_rate_hippo": round(float(row['jockey_win_rate_hippo']) * 100, 1) if pd.notna(row.get('jockey_win_rate_hippo')) else None,
            "ratio_niveau_lot": round(float(row['ratio_niveau_lot']), 2) if pd.notna(row.get('ratio_niveau_lot')) else None,
            "descente_lot": bool(row['descente_lot']) if pd.notna(row.get('descente_lot')) else None,
            "oeilleres_brut": str(row.get('oeilleres_brut', 'SANS_OEILLERES')),
            "driver_win_rate": round(float(row['jockey_win_rate_bayes']) * 100, 1) if pd.notna(row.get('jockey_win_rate_bayes')) else None,
        })
...
def notes_pmu():
    ...
    for p in participants:
        ...
        row = {
            ...
            'plafond_gains':     conditions.get('plafond_gains', 0.0),
            # Cheval
            'age':               p.get('age', 0) or 0,
            'deferre':           _ferrage_map_pmu.get(p.get('deferre', 'FERRE'), 0),
            'deferre_brut':      p.get('deferre', 'FERRE'),
            'oeilleres':         1 if p.get('oeilleres') else 0,
            'oeilleres_brut':    p.get('oeilleres', 'SANS_OEILLERES'),
            'driver':            driver_nom,
            'entraineur':        entr_nom,
    ...
    if 'deferre' in df_nc.columns:
        ...
        # Catégorie ferrure — basée sur deferre_brut (valeur textuelle PMU complète)
        # quand disponible, pour ne pas perdre les cas "PROTEGE_*" qui ne sont
        # pas dans ferrage_map et retombaient à tort sur 'FERRE'.
        df_nc = _appliquer_cat_ferrure(df_nc)
    else:
        df_nc['deferre_4'] = 0.0
    ...
    # ── rk_moyenne_3courses — médiane RK des 3 dernières courses finies ──
    # RK brut sans correction inter-hippodromes (trop de biais par niveau de lot)
    TOLERANCE_DISTANCE = 300
    RK_MIN, RK_MAX = 60000, 100000

    def _rk_moyenne_3courses(row):
        nom = str(row['nom']).upper().strip()
        distance_jour = row.get('distance')
        hist = _rk_distance_cache.get(nom)
        if not hist or pd.isna(distance_jour) or not distance_jour or distance_jour <= 0:
            return None, None
        courses_proches = [
            c['rk'] for c in hist
            if abs(c['distance'] - distance_jour) <= TOLERANCE_DISTANCE
            and RK_MIN <= c['rk'] <= RK_MAX
        ]
        if not courses_proches:
            return None, None
        derniers = sorted(courses_proches[:3])
        n = len(derniers)
        mediane = derniers[n//2] if n%2==1 else (derniers[n//2-1]+derniers[n//2])/2
        return round(mediane, 1), round(derniers[0], 1)

    _res_rk = df_nc.apply(_rk_moyenne_3courses, axis=1)
    df_nc['rk_moyenne_3courses']  = _res_rk.apply(lambda t: t[0])
    df_nc['rk_meilleur_3courses'] = _res_rk.apply(lambda t: t[1])

    # ── rk_corrige — médiane corrigée par hippodrome_code × tranche ──
    # Priorité : hippodrome_code (depuis rk_reference_hippo.json.gz)
    # Fallback  : r_num + niveau_lot (depuis rk_reference.json.gz)
    # Formule : RK_corrigé = RK_brut - ref_origine + ref_jour

    def _tranche(dist):
        if dist <= 1800:   return 'court'
        elif dist <= 2300: return 'moyen'
        elif dist <= 2800: return 'long'
        else:              return 'tres_long'

    def _get_ref_hippo(hippo, tranche):
        """Référence par hippodrome_code — plus précise."""
        return _rk_reference_hippo_cache.get(hippo, {}).get(tranche)

    def _niveau_lot(prix):
        if pd.isna(prix) or prix <= 0: return 'nan'
        if prix > 60000:  return 'A'
        if prix > 35000:  return 'B'
        if prix > 20000:  return 'C'
        if prix > 10000:  return 'D'
        return 'E'

    def _get_ref_fallback(r_str, niveau, tranche):
        """Fallback par r_num + niveau_lot."""
        return _rk_reference_cache.get(str(r_str), {}).get(niveau, {}).get(tranche)

    # Référence de la course du jour
    tranche_jour_glob  = _tranche(df_nc['distance'].iloc[0]) if len(df_nc) > 0 else 'moyen'
    hippo_jour_str     = str(df_nc['hippodrome_code'].iloc[0]) if 'hippodrome_code' in df_nc.columns and len(df_nc) > 0 else ''
    ref_jour_hippo     = _get_ref_hippo(hippo_jour_str, tranche_jour_glob) if hippo_jour_str else None
    r_num_str          = str(r_num)
    montant_jour       = df_nc['montant_prix'].iloc[0] if len(df_nc) > 0 else 0
    niveau_jour        = _niveau_lot(montant_jour)
    ref_jour_fallback  = _get_ref_fallback(r_num_str, niveau_jour, tranche_jour_glob)
    ref_jour           = ref_jour_hippo or ref_jour_fallback  # priorité hippodrome

    def _rk_corrige(row):
        rk_med = row.get('rk_moyenne_3courses')
        if pd.isna(rk_med) or not rk_med:
            return None
        if not ref_jour:
            return rk_med
        nom = str(row['nom']).upper().strip()
        ...
        for c in hist:
            if abs(c['distance'] - distance_jour) > TOLERANCE_DISTANCE:
                continue
            rk_b = c['rk']
            if not (RK_MIN <= rk_b <= RK_MAX):
                continue
            # Référence d'origine — uniquement si hippodrome_code connu
            hippo_o = c.get('hippodrome', '')
            tr_o    = c.get('tranche', tranche_jour_glob)
            ref_o   = _get_ref_hippo(hippo_o, tr_o) if hippo_o else None
            if ref_o and ref_jour:
                corriges.append(rk_b - ref_o + ref_jour)
            else:
                # Hippodrome inconnu → RK brut sans correction
                corriges.append(rk_b)
            if len(corriges) == 3:
                break
        if not corriges:
            return rk_med
        derniers = sorted(corriges)
        n = len(derniers)
        mediane = derniers[n//2] if n%2==1 else (derniers[n//2-1]+derniers[n//2])/2
        return round(mediane, 1)

    df_nc['rk_corrige'] = df_nc.apply(_rk_corrige, axis=1)
    # précédant la course du jour, à distance comparable (± 300m).
    # Même filtre RK_MIN/RK_MAX anti-erreurs que rk_moyenne_3courses.
    from datetime import datetime as _dt
    try:
        date_jour_dt = _dt.strptime(date_str, '%d%m%Y')
    except (ValueError, TypeError):
        date_jour_dt = None

    def _rk_meilleur_an(row):
        if date_jour_dt is None:
            return None, None
        nom = str(row['nom']).upper().strip()
        distance_jour = row.get('distance')
        hist = _rk_distance_cache.get(nom)
        ...
        for c in hist:
            if abs(c['distance'] - distance_jour) > TOLERANCE_DISTANCE:
                continue
            rk_brut = c['rk']
            if not (RK_MIN <= rk_brut <= RK_MAX):
                continue
            try:
                date_course_dt = _dt.strptime(c['date'], '%Y-%m-%d')
            except (ValueError, TypeError):
                continue
            ecart_jours = (date_jour_dt - date_course_dt).days
            if 0 <= ecart_jours <= 365:
                candidats.append((rk_brut, c['date']))
        if not candidats:
            return None, None
        meilleur_rk, meilleure_date = min(candidats, key=lambda x: x[0])
        return round(meilleur_rk, 1), meilleure_date
    _rk_meilleur_resultats = df_nc.apply(_rk_meilleur_an, axis=1)
    df_nc['rk_meilleur_an'] = _rk_meilleur_resultats.apply(lambda t: t[0])
    df_nc['rk_meilleur_an_date'] = _rk_meilleur_resultats.apply(lambda t: t[1])

    # ── date_derniere_course — date de la dernière course connue ──────
    # Source : chrono_cache['date_derniere'] (déjà utilisé par _score_absence).
    def _date_derniere_course(nom):
        entry = _chrono_cache.get(str(nom).upper().strip())
        if entry and isinstance(entry, dict):
            return entry.get('date_derniere') or None
        return None
    df_nc['date_derniere_course'] = df_nc['nom'].apply(_date_derniere_course)

    # ── changement_ferrure — la ferrure du jour diffère-t-elle de celle
    # de la dernière course connue ? (FERRE / DEFERRE_PARTIEL / DEFERRE_TOTAL)
    # Source : _rk_distance_cache[nom][0]['ferrure'] (entrée la plus récente,
    # déjà triée du + récent au + ancien). None si pas d'historique disponible
    # ou si la ferrure de la dernière course n'est pas connue (anciens caches).
    def _changement_ferrure(row):
        nom = str(row['nom']).upper().strip()
        cat_jour = row.get('cat_ferrure')
        hist = _rk_distance_cache.get(nom)
        if not hist or not cat_jour:
            return None, None
        derniere = hist[0]
        cat_precedente = derniere.get('ferrure')
        if not cat_precedente:
            return None, None
        return cat_jour != cat_precedente, cat_precedente
    _res_ferrure = df_nc.apply(_changement_ferrure, axis=1)
    df_nc['changement_ferrure'] = _res_ferrure.apply(lambda t: t[0])
    df_nc['ferrure_precedente'] = _res_ferrure.apply(lambda t: t[1])
    ...
    for _, row in df_nc.sort_values('note_pmu', ascending=False).iterrows():
        result.append({
            ...
            "tendance_chrono": str(row.get('tendance_chrono', 'inconnu')),
            "rk_moyenne_3courses": float(row['rk_moyenne_3courses']) if pd.notna(row.get('rk_moyenne_3courses')) and row.get('rk_moyenne_3courses') else None,
            "rk_meilleur_3courses": float(row['rk_meilleur_3courses']) if pd.notna(row.get('rk_meilleur_3courses')) and row.get('rk_meilleur_3courses') else None,
            "rk_corrige": float(row['rk_corrige']) if pd.notna(row.get('rk_corrige')) and row.get('rk_corrige') else None,
            "gains_carriere": float(row.get('gains_carriere') or 0.0),
            "plafond_gains": float(row.get('plafond_gains') or 0.0),
            "rk_meilleur_an": float(row['rk_meilleur_an']) if pd.notna(row.get('rk_meilleur_an')) and row.get('rk_meilleur_an') else None,
            "rk_meilleur_an_date": str(row.get('rk_meilleur_an_date')) if row.get('rk_meilleur_an_date') else None,
            "date_derniere_course": str(row.get('date_derniere_course')) if row.get('date_derniere_course') else None,
            "driver_win_rate": round(float(row['driver_win_rate_bayes']) * 100, 1) if pd.notna(row.get('driver_win_rate_bayes')) else None,
            "changement_ferrure": bool(row['changement_ferrure']) if row.get('changement_ferrure') is not None else None,
            "ferrure_precedente": str(row.get('ferrure_precedente')) if row.get('ferrure_precedente') else None,
            "gains_carriere": float(row.get('gains_carriere') or 0.0),
            "plafond_gains": float(row.get('plafond_gains') or 0.0),
        })
...
RK_DISTANCE_CACHE_PATH = "rk_distance_cache.json.gz"
# Cache de référence RK par (r_num, tranche_distance) — médiane des 5 premiers
# Utilisé pour calculer RK_corrigée = RK_brut - rk_ref (comparable inter-hippodromes)
RK_REFERENCE_PATH     = "rk_reference.json.gz"
# Nouveau cache de référence par hippodrome_code (plus précis que r_num)
RK_REFERENCE_HIPPO_PATH = "rk_reference_hippo.json.gz"
_rk_reference_hippo_cache = {}  # { "VIN": { "tranche": rk_ref } }
_rk_reference_cache   = {}  # { "r_num": { "tranche": rk_ref } }
# Nom alternatif : extension neutre utilisée pour contourner la décompression
# automatique de certains navigateurs lors de l'upload web vers HuggingFace.
RK_DISTANCE_CACHE_PATH_ALT = "rk_distance_cache.gzdat"
# Cache RK+distance : nom (upper/strip) → liste [{date, distance, rk}, ...] triée du + récent au + ancien
# Construit depuis historique_enrichi.csv (rang_arrivee et reduction_km non-null) — généré en Colab
_rk_distance_cache = {}
# Idem pour le Trot Monté — construit depuis historique_galop_enrichi.csv
# (filtré sur discipline == 'MONTE'), généré en Colab
RK_DISTANCE_CACHE_MONTE_PATH = "rk_distance_cache_monte.json.gz"
RK_DISTANCE_CACHE_MONTE_PATH_ALT = "rk_distance_cache_monte.gzdat"
_rk_distance_cache_monte = {}
# Date de dernière course pour PLAT — { NOM: 'YYYY-MM-DD' }, construit depuis
# historique_galop_enrichi.csv (filtré sur discipline == 'PLAT'), généré en Colab.
# Pas de RK/chrono pour PLAT (non comparable de la même façon qu'en trot).
DATE_DERNIERE_COURSE_PLAT_PATH = "date_derniere_course_plat.json.gz"
DATE_DERNIERE_COURSE_PLAT_PATH_ALT = "date_derniere_course_plat.gzdat"
_date_derniere_course_plat = {}
...
def _notes_pmu_monte_v1(df_nc, date_str, r_num, c_num):
    ...
    # ── rk_moyenne_3courses — médiane RK des 3 dernières courses finies ──
    # RK brut sans correction inter-hippodromes
    TOLERANCE_DISTANCE = 300
    RK_MIN, RK_MAX = 60000, 100000

    def _rk_moyenne_3courses_monte(row):
        nom = str(row['nom']).upper().strip()
        distance_jour = row.get('distance')
        hist = _rk_distance_cache_monte.get(nom)
        if not hist or pd.isna(distance_jour) or not distance_jour or distance_jour <= 0:
            return None, None
        courses_proches = [
            c['rk'] for c in hist
    ...
    _res_rk_m = df_nc.apply(_rk_moyenne_3courses_monte, axis=1)
    df_nc['rk_moyenne_3courses']  = _res_rk_m.apply(lambda t: t[0])
    df_nc['rk_meilleur_3courses'] = _res_rk_m.apply(lambda t: t[1])

    # ── rk_corrige MONTE — même logique que ATTELÉ avec hippodrome_code ──
    def _niveau_lot_m(prix):
        if pd.isna(prix) or prix <= 0: return 'nan'
        if prix > 60000:  return 'A'
        if prix > 35000:  return 'B'
        if prix > 20000:  return 'C'
    ...
    tranche_jour_m     = _tranche_m(df_nc['distance'].iloc[0]) if len(df_nc) > 0 else 'moyen'
    hippo_jour_m_str   = str(df_nc['hippodrome_code'].iloc[0]) if 'hippodrome_code' in df_nc.columns and len(df_nc) > 0 else ''
    ref_jour_m_hippo   = _rk_reference_hippo_cache.get(hippo_jour_m_str, {}).get(tranche_jour_m) if hippo_jour_m_str else None
    r_num_str_m2       = str(r_num)
    montant_jour_m     = df_nc['montant_prix'].iloc[0] if len(df_nc) > 0 else 0
    niveau_jour_m      = _niveau_lot_m(montant_jour_m)
    ref_jour_m_fallback = _rk_reference_cache.get(r_num_str_m2, {}).get(niveau_jour_m, {}).get(tranche_jour_m)
    ref_jour_m         = ref_jour_m_hippo or ref_jour_m_fallback

    def _rk_corrige_monte(row):
        rk_med = row.get('rk_moyenne_3courses')
        if pd.isna(rk_med) or not rk_med:
            return None
        if not ref_jour_m:
        ...
        for c in hist:
            ...
            if not (RK_MIN <= rk_b <= RK_MAX):
                continue
            hippo_o = c.get('hippodrome', '')
            tr_o    = c.get('tranche', tranche_jour_m)
            ref_o   = _rk_reference_hippo_cache.get(hippo_o, {}).get(tr_o) if hippo_o else None
            if ref_o and ref_jour_m:
                corriges.append(rk_b - ref_o + ref_jour_m)
            else:
                # Hippodrome inconnu → RK brut sans correction
                corriges.append(rk_b)
            if len(corriges) == 3:
    ...
    df_nc['rk_corrige'] = df_nc.apply(_rk_corrige_monte, axis=1)
    from datetime import datetime as _dt
    try:
        date_jour_dt = _dt.strptime(date_str, '%d%m%Y')
    except (ValueError, TypeError):
    ...
    def _rk_meilleur_an_monte(row):
        ...
        meilleur_rk, meilleure_date = min(candidats, key=lambda x: x[0])
        return round(meilleur_rk, 1), meilleure_date
    _rk_meilleur_resultats_monte = df_nc.apply(_rk_meilleur_an_monte, axis=1)
    df_nc['rk_meilleur_an'] = _rk_meilleur_resultats_monte.apply(lambda t: t[0])
    df_nc['rk_meilleur_an_date'] = _rk_meilleur_resultats_monte.apply(lambda t: t[1])

    # ── date_derniere_course — depuis chrono_cache_monte ────────────
    def _date_derniere_course_monte(nom):
        entry = _chrono_cache_monte.get(str(nom).upper().strip())
        if entry and isinstance(entry, dict):
            return entry.get('date_derniere') or None
        return None
    df_nc['date_derniere_course'] = df_nc['nom'].apply(_date_derniere_course_monte)

    # ── cat_ferrure + reduction_km_v2_ferrure ────────────────
    df_nc = _appliquer_cat_ferrure(df_nc)

    # ── changement_ferrure — la ferrure du jour diffère-t-elle de celle
    # de la dernière course connue ? Source : _rk_distance_cache_monte.
    def _changement_ferrure_monte(row):
        nom = str(row['nom']).upper().strip()
        cat_jour = row.get('cat_ferrure')
        hist = _rk_distance_cache_monte.get(nom)
        if not hist or not cat_jour:
        ...
        if not cat_precedente:
            return None, None
        return cat_jour != cat_precedente, cat_precedente
    _res_ferrure_monte = df_nc.apply(_changement_ferrure_monte, axis=1)
    df_nc['changement_ferrure'] = _res_ferrure_monte.apply(lambda t: t[0])
    df_nc['ferrure_precedente'] = _res_ferrure_monte.apply(lambda t: t[1])
...
def _charger_stats_plat():
    ...
    try:
        ...
        # Snapshots aptitude terrain — clé apt_terrain_snap (ou aptitude_terrain_snap en compat)
        _apt_ter = to_df('apt_terrain_snap')
        _plat_aptitude_terrain    = _apt_ter if _apt_ter is not None else to_df('aptitude_terrain_snap')
        _plat_apt_type_piste_snap = to_df('apt_type_piste_snap')
        _plat_apt_terrain_label_snap = to_df('apt_terrain_label_snap')
        ...
        n_ter  = len(_plat_aptitude_terrain) if _plat_aptitude_terrain is not None else 0
        n_ahp  = len(_plat_aptitude_hippo_snap) if _plat_aptitude_hippo_snap is not None else 0
        print(f"✅ Snapshots PLAT chargés depuis JSON")
        print(f"   {n_jky} jockeys · {n_entr} entraîneurs · {n_chx} chevaux · {n_hip} jockey×hippo")
        print(f"   {n_ter} entrées apt_terrain · {n_ahp} entrées apt_hippo")
...
def _charger_cache_json_tolerant(path_principal, path_alt, label):
    """
    Charge un cache JSON (dict simple {clé: valeur}) en étant tolérant :
    - au nom de fichier (essaie path_principal puis path_alt),
    - au format réel (gzip ou JSON brut non compressé — certains outils
      d'upload "dégzippent" silencieusement un .gz pendant le transfert).
    Retourne le dict avec les clés normalisées (upper/strip), ou {} si
    le fichier est introuvable ou invalide.
    """
    chemin = None
    if os.path.exists(path_principal):
        chemin = path_principal
    elif os.path.exists(path_alt):
        chemin = path_alt

    if chemin is None:
        print(f"⚠️  {path_principal} (ni {path_alt}) introuvable — {label} désactivé")
        return {}

    try:
        import gzip, json
        print(f"📊 Chargement {label} depuis {chemin}…")
        ...
        if magic == b'\x1f\x8b':
            ...
            print(f"⚠️  {chemin} n'est pas gzippé — lecture en JSON brut")
            with open(chemin, 'r', encoding='utf-8') as f:
                raw = json.load(f)

        result = {k.upper().strip(): v for k, v in raw.items()}
        print(f"✅ {label} chargé : {len(result)} chevaux")
        return result
    except Exception as e:
...
def _charger_rk_distance_cache():
    """
    Charge le cache RK+distance ATTELÉ depuis rk_distance_cache.json.gz (généré
    en Colab à partir de historique_enrichi.csv, filtré sur rang_arrivee +
    reduction_km non-null). Format : { NOM: [{date, distance, rk}, ...] }
    trié du plus récent au plus ancien.
    """
    global _rk_distance_cache
    _rk_distance_cache = _charger_cache_json_tolerant(
        RK_DISTANCE_CACHE_PATH, RK_DISTANCE_CACHE_PATH_ALT, "cache RK+distance ATTELÉ")


def _charger_rk_distance_cache_monte():
    """
    Charge le cache RK+distance MONTE depuis rk_distance_cache_monte.json.gz
    (généré en Colab à partir de historique_galop_enrichi.csv, filtré sur
    discipline == 'MONTE' + rang_arrivee/reduction_km non-null).
    """
    global _rk_distance_cache_monte
    _rk_distance_cache_monte = _charger_cache_json_tolerant(
        RK_DISTANCE_CACHE_MONTE_PATH, RK_DISTANCE_CACHE_MONTE_PATH_ALT, "cache RK+distance MONTE")


def _charger_rk_reference():
    """
    Charge le cache de référence RK par (r_num, tranche_distance).
    Format : { "r_num": { "tranche": rk_ref_median } }
    Utilisé pour normaliser les RK historiques : RK_corrigé = RK_brut - rk_ref
    """
    global _rk_reference_cache
    data = _charger_cache_json_tolerant(RK_REFERENCE_PATH, RK_REFERENCE_PATH, "cache RK référence")
    if data:
        _rk_reference_cache = data
        print(f"   {len(data)} numéros de réunion dans le cache de référence")


def _charger_rk_reference_hippo():
    """Charge le cache de référence RK par hippodrome_code × tranche_distance.
    Format : { "VIN": { "tranche": rk_ref } } — plus précis que r_num."""
    global _rk_reference_hippo_cache
    data = _charger_cache_json_tolerant(RK_REFERENCE_HIPPO_PATH, RK_REFERENCE_HIPPO_PATH, "cache RK référence hippo")
    if data:
        _rk_reference_hippo_cache = data
        print(f"   {len(data)} hippodromes dans le cache de référence hippo")


def _charger_date_derniere_course_plat():
    """
    Charge la date de dernière course PLAT depuis date_derniere_course_plat.json.gz
    (généré en Colab à partir de historique_galop_enrichi.csv, filtré sur
    discipline == 'PLAT'). Format : { NOM: 'YYYY-MM-DD' }.
    """
    global _date_derniere_course_plat
    _date_derniere_course_plat = _charger_cache_json_tolerant(
        DATE_DERNIERE_COURSE_PLAT_PATH, DATE_DERNIERE_COURSE_PLAT_PATH_ALT, "date dernière course PLAT")
...
_charger_rk_distance_cache()
_charger_rk_distance_cache_monte()
_charger_rk_reference()
_charger_rk_reference_hippo()
_charger_date_derniere_course_plat()
